...
pub(crate) mod workflow_support {
    ...
    impl WorkflowCtx {
        ...
        > {
            ...
            let target_execution_id = match ExecutionId::from_str(&execution_id_str) {
                ...
                Ok(ExecutionId::TopLevel(_)) => {
                    return Ok(Err(StubJsonError::ExecutionIdParsingError(
                        "execution-id must be a derived (child) execution ID, not a top-level one".to_string(),
                    )));
                }
                Err(err) => {
                    return Ok(Err(StubJsonError::ExecutionIdParsingError(
                        format!("cannot parse execution-id: {err}"),
                    )));
                }
            };
            ...
            let (fn_metadata, _fn_component_id) = match self
                .event_history
                .fn_registry
                .get_by_exported_function(&target_ffqn)
            {
            ...
            let result = match &return_type.type_wrapper_tl.ok {
                ...
                Some(ok_type) => {
                    let json_val: serde_json::Value = match serde_json::from_str(&result_json) {
                        ...
                        Err(err) => {
                            return Ok(Err(StubJsonError::ResultParsingError(
                                format!("cannot parse result-json: {err}"),
                            )));
                        }
                    };
                    let wast_val_with_type = match deserialize_value(&json_val, ok_type.as_ref().clone()) {
                        Ok(v) => v,
                        Err(err) => {
                            return Ok(Err(StubJsonError::ResultParsingError(
                                format!("result type checking failed: {err}"),
                            )));
                        }
                    };
            ...
            let wast_val = WastVal::try_from(stub_result_val)
                .map_err(|e| wasmtime::Error::msg(format!("stub result conversion error: {e:?}")))?;
            match wast_val {
