...
mod conversions {
    ...
    /// Convert a `DbErrorGeneric` to `rusqlite::Error` for use in row mapping closures.
    #[track_caller]
                ...
                let orphans = tx
                    .prepare(
                        "WITH candidates AS (\
                             ...
                             SUM(size) OVER (ORDER BY digest ROWS UNBOUNDED PRECEDING) AS cumulative_size \
                             FROM candidates\
                         ) \
                    ...
                    .collect::<Result<Vec<_>, _>>()?;
                let candidate_count = orphans.first().map_or(0, |(_, _, count)| *count);
