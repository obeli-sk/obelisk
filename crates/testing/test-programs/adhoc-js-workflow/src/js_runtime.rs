//! JavaScript runtime using Boa engine with Obelisk workflow bindings.
...
pub fn execute(js_code: &str, params: &str) -> Result<String, String> {
    let mut context = Context::default();

    // Set up the obelisk global object
    setup_obelisk_api(&mut context).map_err(|e| format!("Failed to setup API: {e}"))?;
...
fn setup_console(context: &mut Context) -> JsResult<()> {
    ...
    // console.trace
    let trace_fn = NativeFunction::from_fn_ptr(|_this, args, ctx| {
    ...
    console.set(
        js_string!("trace"),
        trace_fn.to_js_function(context.realm()),
        false,
        context,
    )?;

    // console.debug
    let debug_fn = NativeFunction::from_fn_ptr(|_this, args, ctx| {
        let msg = args_to_string(args, ctx)?;
    ...
    console.set(
        js_string!("debug"),
        debug_fn.to_js_function(context.realm()),
        false,
        context,
    )?;
    ...
    console.set(
        js_string!("warn"),
        warn_fn.to_js_function(context.realm()),
        false,
        context,
    )?;
    ...
    console.set(
        js_string!("error"),
        error_fn.to_js_function(context.realm()),
        false,
        context,
    )?;
