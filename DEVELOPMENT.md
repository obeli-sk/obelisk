...
| Task | Files |
|------|-------|
| TOML | `src/config/toml.rs`, `obelisk-help-server.toml`, `obelisk-help-deployment.toml` — update the `obelisk-help-*.toml` files when changing TOML config, then run `scripts/update-toml-schema.sh` to regenerate JSON schemas in `assets/schemas/toml/` |
