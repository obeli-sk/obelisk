...
- *(admin)* Added operator-only gRPC and `/v1/admin` APIs, plus `obelisk admin` commands, for
  ...
  policies that clean up the oldest eligible records first. Explicit deletion can force removal of
  non-terminal trees after verifying that no execution in the tree references the active
  deployment.
...
adds URL support to JavaScript activities and webhooks. It also publishes self-contained Docker
image variants.

### Added

- *(cron)* Cron schedules accept six-field expressions with seconds in addition to five-field
  expressions - ([#880](https://github.com/obeli-sk/obelisk/pull/880)).
- *(cli)* `obelisk generate wit-deps --prune` removes obsolete dependencies generated by Obelisk
  while preserving user-authored WIT files -
  ([#882](https://github.com/obeli-sk/obelisk/pull/882)).
- *(js)* JavaScript activities and webhooks expose the `URL` and `URLSearchParams` globals -
  ([#889](https://github.com/obeli-sk/obelisk/pull/889),
  [#890](https://github.com/obeli-sk/obelisk/pull/890)).
- *(docker)* Self-contained images that bundle the JavaScript runtimes and Web UI are published as
  `ubuntu-embedded`, `ubuntu-litestream-embedded`, and `alpine-embedded` -
  ([#891](https://github.com/obeli-sk/obelisk/pull/891)).

### Changed

- *(sqlite)* Startup uses SQLite's `quick_check` instead of the substantially slower full
  `integrity_check`, retaining page-level structural verification while skipping index/table
  cross-checks - ([#881](https://github.com/obeli-sk/obelisk/pull/881)).
- *(deployment)* Deployment-owned backtrace sources store their generated content digests in the
  component file map, consistently with JavaScript, WASM, and WIT files. Stored 0.41 manifests that
  carry the digest alongside each source path remain supported -
  ([#883](https://github.com/obeli-sk/obelisk/pull/883)).
- *(observability)* Tokio tasks have names when the server is built with `tokio-console` support -
  ([#892](https://github.com/obeli-sk/obelisk/pull/892)).

### Fixed

- *(deployment)* Submission rejects malformed WIT, JavaScript imports missing from the submitted
  package, and declared component files that no module imports. Validation parses submitted sources
  on the server and rejected submissions reclaim their orphaned blobs -
  ([#885](https://github.com/obeli-sk/obelisk/pull/885)).
- *(deployment)* An enqueued deployment is activated only after it compiles and passes verification,
  preventing a failed deployment from being recorded as active -
  ([#886](https://github.com/obeli-sk/obelisk/pull/886)).
- *(security)* Updated `h2` to address
  [RUSTSEC-2026-0258](https://rustsec.org/advisories/RUSTSEC-2026-0258.html) -
  ([#884](https://github.com/obeli-sk/obelisk/pull/884)).
...
### Changed
- Termination is triggered on cancelled activities.
